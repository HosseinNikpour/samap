...
const Contract = (props) => {
    ...
    const getData = () => {
        ...
        getItems('request', { tbName: 'request_vw3', where: '  WHERE (project_state_id = 9)' })
            , checkPermission(9)]).then((response) => {
                if (response[2].data.per_id > 0) {
                    ...
                    dt.forEach(e => {
                        e.production_start_date = e.production_start_date ? moment(e.production_start_date) : undefined;
                        e.production_end_date = e.production_end_date ? moment(e.production_end_date) : undefined;
                        e.letter_date = e.letter_date ? moment(e.letter_date) : undefined;
                        e.support_start_date = e.support_start_date ? moment(e.support_start_date) : undefined;
                        e.support_end_date =   e.support_end_date ?   moment(e.support_end_date) : undefined;
                        
                        
                        //  e.vw_production_duration=`${e.production_duration_m} ماه و ${e.production_duration_d} روز`
                        //  e.vw_support_duration=`${e.support_duration_m} ماه و ${e.support_duration_d} روز`
                    });
    ...
    const saveBtnClick = () => {
        ...
        columns.filter(a => a.req).forEach(a => {
            ...
            if (a.type === 'decimal' && obj[a.accessor] == 0)
                err[a.accessor] = false;
        })
        // err["support_duration_m"] = obj["support_duration_m"] ? false : true;
        // err["support_duration_d"] = obj["support_duration_d"] ? false : true;
        // err["production_duration_m"] = obj["production_duration_m"] ? false : true;
        // err["production_duration_d"] = obj["production_duration_d"] ? false : true;
            ...
            if (mode === 'new') {
                insertItem(formData, entityName, 'multipart/form-data').then(response => {
                    if (response.data.type !== "Error") {
                        let rr = { id: obj.project_id, project_state_id: 10 };
    ...
    const btnDownloadClick = () => {
        convertArrayToTable(data, 'قرارداد ها')
    }
                            ...
                            <div className="row">
                                <div className="col-6">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        {/* <input id='production_price' className={errors.production_price ? "form-control error-control" : "form-control"}
                                            type="number" value={obj.production_price}
                                            onChange={(e) => setObj({ ...obj, production_price: e.target.value })} disabled={mode === 'display'} /> */}
                                        <NumberFormat onValueChange={(e) => setObj({ ...obj, production_price: parseInt(e.value) })}
                                            className={errors.production_price ? "form-control error-control" : "form-control"}
                                            {...Static.numberDefaultProp} disabled={mode === 'display'} value={obj.production_price} />
                                    </div>
                                </div>
                                <div className="col">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        <DatePicker id='production_start_date' onChange={value => setObj({ ...obj, production_start_date: value })}
                                            value={obj.production_start_date} disabled={mode === 'display'} {...Static.datePickerDefaultProp}
                                            className={errors.production_start_date ? "form-control error-control" : 'form-control'} />
                                ...
                                <div className="col">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        <DatePicker id='production_end_date' onChange={value => setObj({ ...obj, production_end_date: value })}
                                            value={obj.production_end_date} disabled={mode === 'display'} {...Static.datePickerDefaultProp}
                                            className={errors.production_end_date ? "form-control error-control" : 'form-control'} />
                            ...
                            <div className="row">
                                <div className="col-6">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        {/* <input id='support_price' className={errors.support_price ? "form-control error-control" : "form-control"}
                                            type="number" value={obj.support_price}
                                            onChange={(e) => setObj({ ...obj, support_price: e.target.value })} disabled={mode === 'display'} /> */}
                                        <NumberFormat onValueChange={(e) => setObj({ ...obj, support_price: parseInt(e.value) })}
                                            className={errors.support_price ? "form-control error-control" : "form-control"}
                                            {...Static.numberDefaultProp} disabled={mode === 'display'} value={obj.support_price} />
                                    </div>
                                </div>
                                <div className="col">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        <DatePicker id='support_start_date' onChange={value => setObj({ ...obj, support_start_date: value })}
                                            value={obj.support_start_date} disabled={mode === 'display'} {...Static.datePickerDefaultProp}
                                            className={errors.support_start_date ? "form-control error-control" : 'form-control'} />
                                ...
                                <div className="col">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        <DatePicker id='support_end_date' onChange={value => setObj({ ...obj, support_end_date: value })}
                                            value={obj.support_end_date} disabled={mode === 'display'} {...Static.datePickerDefaultProp}
                                            className={errors.support_end_date ? "form-control error-control" : 'form-control'} />
