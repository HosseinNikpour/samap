...
const ContractDraft = (props) => {
    ...
    const getData = () => {
        ...
        Promise.all([getItems(entityName), getItems('request', { tbName: 'request_vw3', where: '  WHERE (project_state_id = 8)' })
            , checkPermission(8)]).then((response) => {
    ...
    const btnNewClick = () => {
        setMode('new');
        setObj({ ...obj, deadline: moment().add(30, 'days') })
            ...
            if (mode === 'new') {
                insertItem(formData, entityName, 'multipart/form-data').then(response => {
                    if (response.data.type !== "Error") {
                        let rr = { id: obj.project_id, project_state_id: 9 };
    ...
    const displayBtnClick = (item) => {
        setMode('display');
        BoxRef.current.scrollIntoView({ behavior: 'smooth' });
        //item.deadline=item.deadline.format('jYYYY/jMM/jDD')
        setObj(item);
